...
<h1 class="page-title"><el-icon><Clock/></el-icon><span>时光轴</span></h1>
<p class="page-subtitle">记录生活中的每一个美好瞬间</p>
</div>
<div class="timeline-container" v-loading="loading">
<el-timeline class="timeline">
<el-timeline-item v-for="record in records" :key="record.id" :timestamp="formatDate(record.created_at)" placement="top" :color="getTypeColor(record.type)" size="large">
...
<el-tag :type="getTypeTag(record.type)" size="small" effect="plain">{{getTypeLabel(record.type)}}</el-tag>
...
<el-tag v-for="tag in getTags(record.tags).slice(0,3)" :key="tag" size="small" class="tag-item">{{tag}}</el-tag>
</div>
</div>
</div>
...
<el-button class="btn-delete" size="small" @click.stop="confirmDelete(record)"><el-icon><Delete/></el-icon>删除</el-button>
</div>
</el-card>
</el-timeline-item>
</el-timeline>
...
import {Clock,Document,View,Delete,Picture} from "@element-plus/icons-vue"
...
const updateDialogWidth=()=>{dialogWidth.value=window.innerWidth<=768?"92%":"750px"}
updateDialogWidth()
...
const loadRecords=async()=>{loading.value=true;try{records.value=await getRecords()}catch(e){}finally{loading.value=false}}
const formatDate=d=>new Date(d).toLocaleDateString("zh-CN",{year:"numeric",month:"long",day:"numeric"})
const formatDateTime=d=>new Date(d).toLocaleString("zh-CN",{year:"numeric",month:"long",day:"numeric",hour:"2-digit",minute:"2-digit"})
...
const getBrief=t=>{if(!t)return"暂无内容";return t.length>80?t.substring(0,80)+"...":t}

const viewRecord=r=>{currentRecord.value=r;isEdit.value=false;dialogVisible.value=true}
const goUpload=()=>router.push("/upload")

const saveEdit=async()=>{
  if(!editForm.value.title.trim()){ElMessage.warning("请输入标题");return}
  saving.value=true
  try{
    await updateRecord(currentRecord.value.id,{type:currentRecord.value.type,title:editForm.value.title,content:editForm.value.content,tags:editForm.value.tags})
    ElMessage.success("保存成功");dialogVisible.value=false;loadRecords()
...
const confirmDelete=async(record)=>{
  try{
    ...
    await deleteRecord(record.id);ElMessage.success("删除成功");loadRecords()
  }catch(e){if(e!=="cancel")console.error(e)}
}
</script>

<style scoped>
.timeline-page{min-height:calc(100vh - 56px);background:linear-gradient(180deg,#f5f7fa 0%,#e4e8ec 100%);padding:24px 12px 40px}
.timeline-header{text-align:center;margin-bottom:30px}
.page-title{font-size:24px;color:#303133;margin-bottom:6px;display:flex;align-items:center;justify-content:center;gap:8px}
.page-subtitle{font-size:13px;color:#909399}
.timeline-container{max-width:750px;margin:0 auto}
.timeline{padding:10px 0}
.record-card{border-radius:10px;transition:all 0.25s;margin-left:16px;cursor:pointer}
.record-card:hover{transform:translateX(4px);box-shadow:0 8px 24px rgba(0,0,0,0.08)}
.record-content-wrapper{display:flex;gap:16px}
.record-thumb{width:140px;height:100px;flex-shrink:0;border-radius:8px;overflow:hidden;background:#f0f2f5;position:relative}
.record-thumb-text{display:flex;align-items:center;justify-content:center;background:#e9ecef}
.text-icon{font-size:48px;color:#c0c4cc}
.thumb-image{width:100%;height:100%;object-fit:cover}
.image-count{position:absolute;bottom:4px;right:4px;background:rgba(0,0,0,0.6);color:#fff;border-radius:12px;padding:2px 8px;font-size:11px;display:flex;align-items:center;gap:3px}
.image-count .el-icon{font-size:12px}
.record-info{flex:1;min-width:0}
.record-meta{display:flex;align-items:center;gap:10px;margin-bottom:8px}
.record-time{font-size:12px;color:#909399}
.record-title{font-size:15px;color:#303133;margin-bottom:6px;line-height:1.4;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.record-brief{font-size:13px;color:#606266;line-height:1.6;margin-bottom:8px;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical;overflow:hidden}
.record-tags{display:flex;flex-wrap:wrap;gap:6px}
.tag-item{background:#f5f7fa;color:#667eea;border-color:#e4e7ed;font-size:11px}
...
.timeline-page{padding:16px 8px 24px;min-height:calc(100vh - 50px)}
.page-title{font-size:20px}
.page-subtitle{font-size:12px}
.record-content-wrapper{flex-direction:column}
.record-thumb{width:100%;height:180px}
.record-card{margin-left:8px}
